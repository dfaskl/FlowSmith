...
import com.flowsmith.config.JwtSecretProvider;
...
public class AuthService {
     ...
     */
    @Value("${flowsmith.default-username:}")
    private String defaultUsername;

    /**
     * 默认密码（通过环境变量配置，不配置则禁用默认账户）
     */
    @Value("${flowsmith.default-password:}")
    ...
    @Value("${flowsmith.auth.access-token-expiration-minutes:120}")
    private long accessTokenExpirationMinutes;

    @Value("${flowsmith.auth.refresh-token-expiration-hours:168}")
